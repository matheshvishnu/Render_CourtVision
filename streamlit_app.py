...
class VideoProcessor:
    def __init__(self, detector_path, pose_path=None, device="cpu",
        ...
        with st.spinner("📥 Downloading/Loading YOLO model..."):
            ...
            try:
                ...
                pass
        st.toast("✅ YOLO model loaded successfully.", icon="🤖")
    ...
    def process(self, input_path, output_path, progress_callback=None):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to initialize imageio writer. Is 'imageio-ffmpeg' installed? Error: {e}")
...
HUGGINGFACE_MODEL_URL = "https://huggingface.co/matheshvishnu/courtvision-best-pt/resolve/main/best.pt"
...
with col2:
    ...
    use_hf_model = st.checkbox("Use Hugging Face hosted model", value=True)
    
    uploaded_model = None
    if not use_hf_model:
        uploaded_model = st.file_uploader("Upload detector `.pt`", type=["pt"])
    
    pose_file = st.file_uploader("Optional: upload pose `.pt`", type=["pt"])
...
if st.button("🚀 Start Analysis", use_container_width=True):
    ...
    if uploaded_video:
        ...
        with st.spinner("Starting analysis... Please wait. This may take a few minutes."):
            ...
            detector_path = ""
            if use_hf_model:
                detector_path = HUGGINGFACE_MODEL_URL
            elif uploaded_model:
                ...
                st.error("Please select the Hugging Face model or upload your own.")
                st.stop()

            pose_path = None
            if pose_file:
                pose_tmp = tmp_dir / f"pose_{pose_file.name}"
                with open(pose_tmp, "wb") as f:
                    f.write(pose_file.getbuffer())
